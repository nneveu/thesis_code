...
for n, i in enumerate(range(200, 275, 5)): #250, 5)):
   ...
   (dx, dy, Nframes, image_array) = readimage(yag, header_size=5)
   ...
   di_images = do_filter(charge_images, n=3) #no_background, n=3)

   #Crop images 
   x, y, z = di_images.shape
   print(x,y,z)
   #usage = np.zeros((dy, dx, dz))
   crop_array = np.zeros((500,400,z)) 
   for j in range(0,z):
      crop_array[:,:,j] = crop_image(di_images[:,:,j], x_min=80, x_max=480, y_min=120, y_max=620)
   ...
   add_dist_to_image(ave_crop, fiducial, './output/'+key+'_M'+mval,title="M="+mval, background=10)
   ...
   beamsizes, rx2, ry2 = combo_model(crop_array, fiducial, './output/40nC/beamsizes_'+key+'_M'+mval+'.pdf')
