...
data_directory  = './sol_scan_nov-2017/low_charge_BFscan_M260_data' #high_charge_Mscan_data' #low_charge_Mscan_data/
...
circle_dim = circle_finder(fid_image, min_r=0.34, max_r=0.35)
#test = crop_image(fid_image, x_min=80, x_max=400, y_min=80, y_max=400)
...
summary = 'beamsizes_BFscan_M260_low_charge_11-02-2018.txt'
f = open(summary,'w')
f.write('Npoints' + "\t"+ 'Mvalue' + "\t" + 'sigmax_ave' + "\t" + 'sigmay_ave' + "\t" \
        + 'stdx' + "\t" + 'stdy' + "\t" + 'rx_ave' + "\t"+ 'ry_ave'+"\n")

bfvals = np.array([300,350,370,390,410,430,450,470,500,525,550])

for n, i in enumerate(bfvals[0:2]):  #range(200, 205, 5)): #250, 5)):
   print('\n\n')
   #mval = str(i)
   bfval = str(i)
   yag       = glob(data_directory+'/YAG1_M*BF'+bfval+'*2017_img.dat')[0]
   ict_file  = glob(data_directory+'/YAG1_M*BF'+bfval+'*.csv')[0] 
   yag_back  = glob(data_directory+'/YAG1_M*BF'+bfval+'*background*.dat')[0]
