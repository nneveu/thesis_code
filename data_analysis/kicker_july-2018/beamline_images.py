...
for n, i in enumerate(yag_nums):
   ...
   #Load fiducials
   fiducial = np.loadtxt('beamline_fid.txt', skiprows=1)[n][1]
   print(fiducial)
   #-------------------------------------------------------------------------
   #Get charges 
   ...
   #usage = np.zeros((dy, dx, dz))
   crop_array = np.zeros((225,300,z)) 
   for j in range(0,z):
      crop_array[:,:,j] = crop_image(di_images[:,:,j], x_min=100, x_max=400, y_min=150, y_max=375)
